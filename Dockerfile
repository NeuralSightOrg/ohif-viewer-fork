...
RUN yarn install --frozen-lockfile true --verbose || (echo "Yarn install failed" && exit 1)

COPY . .

# To restore workspaces symlinks
RUN yarn install --frozen-lockfile true --verbose || (echo "Yarn install (symlinks) failed" && exit 1)
